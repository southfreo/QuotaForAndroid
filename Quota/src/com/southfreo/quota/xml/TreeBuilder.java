...
public TreeNode parseXML( InputStream reader )
    throws XmlPullParserException, IOException
{
    TreeNode root = null;
    mParser = getParser();
    if (mParser != null) {
        mParser.setInput( reader,"UTF-8");
        root = buildTree();
    }
    return root;
}

//public TreeNode parseXML( XmlResourceParser parser )
public TreeNode parseXML( XmlPullParser parser )
throws XmlPullParserException, IOException
{
	TreeNode root = null;
	mParser = parser;
